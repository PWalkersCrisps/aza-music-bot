...
module.exports = {
    ...
    async execute(client: any, interaction: any, player: any) {
        if (!(interaction.user.id === '426455031571677197')) return interaction.reply({ content: `${interaction.user} actually have permissions to use the command next time`, ephemeral: true });
